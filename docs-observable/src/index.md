...
Circuits are defined in KDL, a semantic format that describes **topology** — what components exist, how they connect, and what role each plays. The KDL carries more information than a SPICE netlist — component roles, group topologies, and placement hints enable the Ferrite layout engine to produce readable schematics automatically. The engine reads the KDL, runs a topology-aware layout algorithm, and produces an SVG circuit diagram — all in WebAssembly. No pre-rendered images, no manual coordinates.

When you change a parameter, the KDL updates, the layout engine re-renders the SVG, the simulator re-runs, and the results are overlaid as annotations directly on the schematic. This **topology** (KDL) → **rendering** (SVG) → **simulation** (spice-rs) pipeline means every example on this site is a live, editable experiment.
